...
class PointSetRegistration:
    def __init__(self, ref_points, tmp_points, alpha, beta, R_star, P_star):
    ...
    def solve(self, K, maxiter=15, simulated_anneal=True):
        ...
        alpha_rot = self.alpha * np.linalg.norm(self.tmp_points, ord='fro') ** 2  # Penalty factor for rotation
        alpha_perm = self.beta * np.linalg.norm(self.ref_points, ord='fro') ** 2 # Penalty factor for permutation
...
def example_2d():
    ...
    # Start solver
    psr = PointSetRegistration(ref_points=ref_points, tmp_points=tmp_points, alpha=alpha, beta=beta, R_star=R_star, P_star=P_star)
...
def example_3d():
    ...
    # Start solver
    psr = PointSetRegistration(ref_points=ref_points, tmp_points=tmp_points, alpha=alpha, beta=beta, R_star=R_star, P_star=P_star)
    history = psr.solve(K=5, maxiter=15, simulated_anneal=True)  # Adjust alpha_rot
