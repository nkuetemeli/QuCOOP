...
class QAP:
    def __init__(self, Q, alpha, P_star):
        self.Q = Q
        self.alpha = alpha
        self.P_star = P_star
    ...
    def solve(self, maxiter=15, simulated_anneal=True):
        ...
        Q = self.Q - self.alpha * np.eye(self.n*self.n)  # Q-matrix with penalty term
        ...
        for j in range(maxiter):
            ...
            x0 = q
            P_sol = (Pc + dPx0 @ x0).reshape(self.n, self.n)
            sol = self.objective(P_sol)
            history.append(sol)
...
if __name__ == '__main__':
    ...
    Q = -np.kron(A, B)
    alpha = np.min(np.linalg.eigvalsh(Q))  # Penalty factor to penalize non-valid matrices

    # Set seed back to None
    np.random.seed(None)

    # Start Solver
    qap = QAP(Q, alpha, P_star)
    history = qap.solve()
