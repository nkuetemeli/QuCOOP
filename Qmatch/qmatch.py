...
def RunAnneal(W, b):
    ...
    response = sampler.sample_ising(bias, J, auto_scale=True, chain_strength=chain, num_reads=50,
                                    return_embedding=True)  # anneal_schedule=((0.0,0.0),(40.0,0.5),(140.0,0.5),(180.0,1.0)))
...
def RunAnnealSim(sweeps, num_reads, W, b):
...
def optimize(sweeps, num_reads, W, initial, CycleSuggestions, iterationNumber, problem):
    """
    ...
    for i in range(len(CycleLists)):
        ...
        # [Result,info]= RunAnneal(We,ce)
        [Result, info] = RunAnnealSim(sweeps, num_reads, We, ce)
        change = Result
